...
class SystemTimer():
    ...
    def __init__(self, end_time):
        self.end_time=Decimal(str(end_time)) # in us
        self.current_time=Decimal(str(0)) # in us
        self.slot_time=Decimal(str(52)) #in us, 24.3.14 in the draft 5.0
        self.SIFS=Decimal(str(160)) # in us, https://mentor.ieee.org/802.11/dcn/12/11-12-1104-02-00ah-11ah-interframe-spacing-values.pptx
    ...
    def register_event(self,event,print_on=False): # register an event in the timeline
        assert event.time>=self.current_time, "current time %r, event.time %r" % (self.current_time,event.time)
        import copy
