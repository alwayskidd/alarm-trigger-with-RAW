...
class  AP(device.Device): # has no  downlink traffic there
    ...
    #	False--channel status is not changed
        ...
        if self.packet_can_receive==None or (self.packet_can_receive in packets_in_air): # the current can receive packet is still transmitting
    ...
    # This function is called when a RAW slot starts
        ...
        self.current_slot.status=self.channel_state
