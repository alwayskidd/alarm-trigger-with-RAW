...
class Sensor(device.Device):
	...
	#	False--this senosr is not backing off
		...
		if self.access_mode=="General RAW": # calculate the expected time to transmit a data frame and get the ack
			expected_time=(self.timer.slot_time*backoff_timer+self.queue[0].transmission_delay+self.timer.SIFS
				+self.timer.NDP_time)
		elif self.channel_state=="Trigger RAW": # calculate the expected time to transmit a PS-POLL frame and get the ACK
			expected_time=(self.timer.slot_time*backoff_timer+self.timer.NDP_time*2+self.timer.SIFS)
		if self.access_mode=="General RAW" or self.access_mode=="Trigger RAW":
			if self.timer.current_time+expected_time>self.invovled_RAW_slot.end_time:
		...
		if self.backoff_timer<=0: # transmit this packet immediately
			if self.access_mode=="Trigger RAW": # transmit a ps-poll frame
				packet=packet.Packet(self.timer,"NDP Ps-poll",self,[self.AP])
	...
	# This function is called when the sensor received a Beacon frame from AP
		...
		for each_RAW in beacon:# check whether the STA is in a certain RAW
			if ((each_RAW.paged_only and self in each_RAW.paged_STAs) or 
				...
				for each_slot in each_RAW:
					if self in each_slot.STAs:
						self.next_RAW_slot=each_slot
						break
			if each_RAW.end_time>end_time_for_all: #calculate when the next open access will start
				end_time_for_all=each_RAW.end_time
		self.status="Sleep"
		if self.next_RAW_slot!=None: # wake up at certain time for this RAW
			new_event=event.Event("Wakeup for RAW", self.invovled_RAW_slot.start_time)
			new_event.register_device(self)
			self.timer.register_event(new_event)
			return True
		else: # wake up while the channel is open for access
			self.next_open_access=end_time_for_all
	...
	#	False--the sensor has nothing to report
		...
		if self.channel_state=="Idle": # start backoff after DIFS
			new_event=event.Event("IFS expire",self.timer.DIFS)
			new_event.register_device(self)
			self.timer.register_event(new_event)
		new_event=event.Event("Endup RAW",self.next_RAW_slot.end_time) # end this RAW
		new_event.register_event(self)
		self.timer.register_event(new_event)
		return True
	...
	#	False--the sensor keeps sleep
		if not self.queue:
			return False
		assert self.next_open_access==self.timer.current_time
